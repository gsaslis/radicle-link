...
fn setup_path() -> anyhow::Result<PathBuf> {
    // The git-remote-rad binary is located under radicle-link/target/debug
    let helper_path = env!("CARGO_MANIFEST_DIR");
    ...
    if !helper_path.join("git-remote-rad").exists() {
        std::process::Command::new("cargo")
            .args(&["build", "--bin", "git-remote-rad"])
            .output()?;
    }
