...
pub enum Update<'a> {
    ...
    /// A no-op update asserts that the `expect` value is still the same.
    ///
    /// Useful to ensure nothing changed between two transactions.
    Noop {
        name: Cow<'a, BStr>,
        expect: ObjectId,
    },
...
impl Update<'_> {
    ...
    pub fn into_owned<'b>(self) -> Update<'b> {
        match self {
            ...
            Self::Noop { name, expect } => Update::Noop {
                name: Cow::from(name.into_owned()),
                expect,
            },
        }
