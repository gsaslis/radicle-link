    ...
    Oid: Debug + AsRef<oid>,
{
    ...
    for (peer, refs) in &sigrefs.refs {
        ...
        let prefix = PathBuf::from("refs").join("remotes").join(peer.to_string());

        info!("scanning {} for signed refs", prefix.display());
    ...
    // unsigned remote tracking
    {
        ...
        for peer in &sigrefs.remotes {
            ...
            info!(prefix = %prefix.display(), "scanning for unsigned trackings");
    ...
    // finally, find orphans and other strange refs
    {
        ...
        for item in Refdb::scan::<_, PathBuf>(cx, None)? {
            let (name, _oid) = item?;
