...
pub trait Identities {
    ...
    /// Verify the identity history with URN `urn`.
    ///
    /// Similar to [`Identities::verify`], but resolves the tip by examining the
    /// ref described by the URN.
    fn verify_urn<F, T>(
        &self,
        urn: &Self::Urn,
        resolve: F,
...
pub fn newest<'a, C, I>(cx: &C, of: I) -> Result<Option<C::VerifiedIdentity>, Error>
where
    ...
    I: IntoIterator<Item = &'a PeerId>,
{
    ...
    for id in of {
        let a = if id == ours {
            ...
            self::of(cx, id)?.ok_or(format!("newest: missing `refs/remotes/{}/rad/id`", id))?
        };
        match newest {
            None => newest = Some(a),
            Some(b) => newest = Some(Identities::newer(cx, a, b)?),
        }
    }
